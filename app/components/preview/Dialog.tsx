  ...
  return (
    ...
    >
      <div
        ...
        className="md:w-[96%] md:pb-0 pb-12 overflow-auto md:h-[96%] w-full h-full transition-all duration-150 bg-white rounded-2xl border-4 border-black shadow-active flex lg:flex-row flex-col"
      >
        <div className="lg:w-[300px] relative pt-12 p-6 items-center justify-around lg:justify-start flex gap-14 lg:flex-col md:flex-row flex-col bg-gradient-to-b from-gray-100 to-white border-black lg:border-r-4 border-b-4 lg:border-b-0 lg:!rounded-l-2xl rounded-t-2xl lg:!rounded-t-none">
          ...
          <img
            ...
            alt="mascot"
            src="/mascot.svg"
            className="fixed z-20 lg:w-12 w-8 -rotate-3 hover:-rotate-12 top-8 left-8 md:left-16 md:top-12 transition-all duration-150 animate-fadeIn opacity-0"
          />
          <div className="flex flex-col min-w-[250px]">
            <div className="border-b flex justify-between items-baseline mt-8 border-black pl-4">
              ...
              <span className="px-4 py-2 bg-black text-white font-mono">
                {analysis.summary?.confidence?.toFixed(2)}%
              </span>
            </div>
            ...
            <div className="border-b flex justify-between items-baseline mt-8 border-black pl-4">
              ...
              <span className="px-4 py-2 bg-black text-white font-mono">
                {analysis.workExperience?.confidence?.toFixed(2)}%
              </span>
            </div>
            ...
            <div className="border-b flex justify-between items-baseline mt-8 border-black pl-4">
              ...
              <span className="px-4 py-2 bg-black text-white font-mono">
                {analysis.skills?.score?.toFixed(2)}%
              </span>
            </div>
            ...
            <div className="border-b flex justify-between items-baseline mt-8 border-black pl-4">
              ...
              <span className="px-4 py-2 bg-black text-white font-mono">
                {analysis.projects?.confidence?.toFixed(2)}%
              </span>
            </div>
          </div>
          ...
          <div className="lg:flex flex-row lg:absolute hidden bottom-3 gap-3 right-3 z-50">
            <button
              onClick={onClose}
              className="px-4 py-2 bg-white border border-black text-black rounded-xl font-semibold"
            >
              {!text ? "Close" : "Edit"}
            </button>
            {!text && (
              <button
                onClick={handlePrint}
                className="px-4 py-2 bg-black border border-black text-white font-semibold rounded-xl"
              >
                Download
              </button>
            )}
        ...
        <div className="p-6 flex flex-col gap-12 w-full lg:overflow-auto">
          <div className="flex flex-col gap-4">
            <span className="flex gap-2 text-2xl font-semibold items-center">
              <BiAnalyse /> Analysis
            </span>
            <div className="flex flex-row gap-3 justify-between md:flex-nowrap flex-wrap">
              <div className="border-b flex justify-between items-baseline w-full border-black pl-4">
                ...
                <span className="px-4 py-2 bg-black text-white font-mono">
                  {analysis.semanticAnalysis?.score?.toFixed(2)}%
                </span>
              </div>
              ...
              <div className="border-b flex justify-between items-baseline w-full border-black pl-4">
                ...
                <span className="px-4 py-2 bg-black text-white font-mono">
                  {analysis.sectionCompleteness?.completenessScore?.toFixed(2)}%
                </span>
              </div>
              ...
              <div className="border-b flex justify-between items-baseline w-full border-black pl-4">
                ...
                <span className="px-4 py-2 bg-black text-white font-mono">
                  {analysis.keywordMatching.matchScore}%
                </span>
              </div>
            </div>
          </div>
          <div className="flex flex-col gap-1 border-b border-black">
            <div className="flex flex-row gap-6 justify-between items-center">
              <span className="flex gap-2 text-2xl font-semibold items-center">
                <CgFormatLeft /> Formatting
              </span>
              <span className="px-4 py-2 bg-black text-white font-mono">
                {analysis.formatting?.score?.toFixed(2)}%
              </span>
            </div>
            {analysis.formatting?.issues?.[0] && (
              <div className="ml-6 relative pl-6 pb-3 pt-3 border-l-2 border-black">
                {analysis.formatting.issues.map((issue, index) => (
                  <span
                    key={index}
                    className="ml-2 before:content-[''] before:w-2 before:h-2 before:ml-3 before:mt-2 before:bg-black before:flex before:absolute before:left-0 text-base"
                  >
                    {issue}
                  </span>
                ))}
          ...
          <div className="flex flex-col gap-4 mt-8">
            ...
            <div className="flex flex-col gap-8">
              {analysis.suggestions ? (
                      ...
                      <div key={index} className="flex flex-col gap-2">
                        ...
                        <div className="pl-6 border-l-4 border-black relative flex flex-col gap-5">
                          {value.map((suggestion, subIndex) => (
                            <div
                              key={subIndex}
                              className="ml-2 before:content-[''] py-1 text-justify before:w-2 before:h-2 before:ml-3 before:mt-2 before:bg-black before:flex before:absolute before:left-0 text-base font-medium"
                            >
                              {suggestion}
                            </div>
                          ))}
                ...
                <div className="w-full min-h-96 h-fit relative rounded-2xl border-2 border-black flex items-center justify-center">
                  <div className="absolute left-0 backdrop-blur-lg blur-lg top-0 w-full h-full bg-gradient-to-r from-white via-black to-white bg-[length:200%_200%] animate-glitter rounded-2xl opacity-10" />
                  <p className="absolute top-2 left-2 blur-sm select-none">
                    You know how good cvbuildr is? WOAHHHHHH You can literally
                    build your resume for free without any hassle!
                    <br />
                    {generateRandomCharacters(1900)}
                  </p>
                  <Link
                    href="/pricing"
                    className="px-6 py-2 z-10 rounded-xl bg-black text-white flex gap-2 items-center justify-center"
                  >
                    <TbCrown /> Buy Pro
                  </Link>
                </div>
...
const generateRandomCharacters = (length: number) => {
  const characters =
    "ABCD EFG HIJKLM NOPQRS TUVWXYZab cdefghijk lmnop qrstuvwx yz0123 456789";
  let result = "";
  const charactersLength = characters.length;
  const newLineChance = 0.1; // 10% chance for a new line
  for (let i = 0; i < length; i++) {
    if (Math.random() <= newLineChance) {
      result += "\n";
    } else {
